...
> ⚠️ **Buena práctica:** Antes de ejecutar un `DELETE` masivo, lanzar primero el `SELECT` equivalente para revisar exactamente qué filas se eliminarán.
...
Oracle usa **bloqueos a nivel de fila** (*row-level locking*). T1 adquiere un bloqueo exclusivo sobre la fila `id=1`. T2 debe esperar hasta que T1 libere ese bloqueo con `COMMIT` o `ROLLBACK`. Esto garantiza el principio de **Aislamiento** (la I de ACID) y evita lecturas sucias.
...
- **DML (INSERT / UPDATE / DELETE)** son las herramientas de manipulación de datos; siempre operan dentro de una transacción.
- **TCL (COMMIT / ROLLBACK / SAVEPOINT)** controlan la durabilidad y la atomicidad de esas operaciones.
- Los **bloqueos de fila** en Oracle aseguran la consistencia en entornos concurrentes sin bloquear filas no afectadas.
- El **DDL implica COMMIT**: mezclar DDL y DML en la misma sesión sin planificación puede causar pérdidas de datos irrecuperables.
